...
static int parse_playlist(AppleHTTPContext *c, const char *url, const char *headers,
                          struct variant *var, AVIOContext *in)
{
    int ret = 0, duration = 0, is_segment = 0, is_variant = 0, bandwidth = 0, is_discontinuity = 0;
    char line[1024];
    const char *ptr;
    int close_in = 0;
    ...
    if (!in) {
        close_in = 1;
        if ((ret = avio_open2(&in, url, headers, AVIO_RDONLY)) < 0)
            return ret;
    }
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
           ...
           is_discontinuity = 1; 
            ...
            if (is_segment) {
                ...
                //discard the discontinuity of the first segment
                if(var->n_segments > 0)
...
reload:
        ...
        if (!v->finished &&
                ...
                (ret = parse_playlist(c, v->url, v->parent->headers, v, NULL)) < 0)
            return ret;
        ...
        if (v->segments[v->cur_seq_no - v->start_seq_no]->discontinuity)
            v->discontinuity = 1;
    }
...
static int applehttp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    if ((ret = parse_playlist(c, s->filename, s->headers, NULL, s->pb)) < 0)
        goto fail;
    ...
    if (c->n_variants > 1 || c->variants[0]->n_segments == 0) {
        for (i = 0; i < c->n_variants; i++) {
            ...
            if ((ret = parse_playlist(c, v->url, s->headers, v, NULL)) < 0)
                goto fail;
        }
    }
...
start:
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        if (var->needed && !var->pkt.data) {
            ...
            int idx = var->cur_seq_no - var->start_seq_no;
            if(var->discontinuity) {
                var->discontinuity = 0;
                var->pkt.flags |= AV_PKT_FLAG_DISCONTINUE;
            }
        }
