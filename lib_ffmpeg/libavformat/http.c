        ...
        *p = '\0';
        ...
        if (!strcasecmp(tag, "Location")) {
            ...
            *new_location = 1;
            ...
            int64_t last_pos = 0;
            int64_t instance_length = 0;
            if (!strncmp (p, "bytes ", 6)) {
                p += 6;
                s->off = atoll(p);
                if ((dash = strchr(p, '-')) && strlen(dash) > 0)
                    last_pos = atoll(dash+1);
                if ((slash = strchr(p, '/')) && strlen(slash) > 0)
                    instance_length = atoll(slash+1);

                // Refer to HTTP/1.1 Section 14.16
                if (instance_length > last_pos)
                    s->filesize = instance_length;
            }
...
static int http_read(URLContext *h, uint8_t *buf, int size)
{
    HTTPContext *s = h->priv_data;
    int len;
    //printf("http_read, chunksize %lld\n", s->chunksize);
