...
int ffurl_open2(URLContext **puc, const char *filename, const char *headers, int flags)
{
    int ret = ffurl_alloc(puc, filename, flags);
    if (ret)
        return ret;
    if (*puc && headers && !strcmp((*puc)->prot->name, "http")) {
        ff_http_set_headers(*puc, headers);
    }
    ret = ffurl_connect(*puc);
    if (!ret)
        return 0;
                                        ...
                                        transfer_func_t transfer_func)
{
    ...
    int fast_retries = 5;
    int retries = 10;
    const unsigned int MAX_LOOP_TIME = 500; // ms
    ...
    len = 0;
    while (len < size_min) {
        //printf("call transfer_func %d\n", size-len);
        ret = transfer_func(h, buf+len, size-len);
        //printf("call transfer_func ok , ret %d\n", ret);
        ...
        unsigned int consumed_ms = (tv2.sec - tv1.sec)*1000 + tv2.usec/1000 - tv1.usec/1000;
        if (retries <= 0 || consumed_ms > MAX_LOOP_TIME) {
...
int ffurl_close(URLContext *h)
{
    ...
    ff_network_close();
#endif
    if (h->prot && h->prot->priv_data_size && h->priv_data){
        av_free(h->priv_data);
    }
    av_free(h);
    return ret;
