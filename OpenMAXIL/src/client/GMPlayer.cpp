        ...
        OMX_TICKS position)
{
    ...
    if(bSetDelayedSurface == OMX_TRUE){
        printf("in Seek(), call SetVideoSurface");
        bAudioMarkDone = OMX_TRUE;
        bVideoMarkDone = OMX_TRUE;
        bSetDelayedSurface = OMX_FALSE;
        ret = SetVideoSurface(DelayedSurface, DelayedSurfaceType);
    }
...
OMX_ERRORTYPE GMPlayer::SetVideoSurface(OMX_PTR surface, OMX_VIDEO_SURFACE_TYPE type)
{
    ...
    LOG_DEBUG("SetVideoSurface: %p, bStreamingSource %d, bBuffering %d, bAudioMarkDone %d, bVideoMarkDone %d\n", 
        mSurface, bStreamingSource, bBuffering, bAudioMarkDone, bVideoMarkDone);

    // if it's streaming , network is bad and player blocked in seeking
    if(bStreamingSource == OMX_TRUE && bBuffering == OMX_TRUE && (bAudioMarkDone == OMX_FALSE || bVideoMarkDone == OMX_FALSE)) {
