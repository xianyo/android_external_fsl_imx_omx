...
OMX_ERRORTYPE GMPlayer::SetVideoSurface(OMX_PTR surface, OMX_VIDEO_SURFACE_TYPE type)
{
    ...
    //in case that SetVideoSurface() is called during Load(), to avoid video be disabled because of surface is unavailable
    if(State == GM_STATE_LOADING){
        LOG_DEBUG("Assign to mSurface in loading state %p", surface);
        mSurface = surface;
        mSurfaceType = type;
    }
