...
LibavVideoDec::LibavVideoDec()
{
    ...
    bNeedCopyCodecDataToDataBuffer = OMX_FALSE;
...
OMX_ERRORTYPE  LibavVideoDec::SetRoleFormat(OMX_STRING role)
{
    if(fsl_osal_strcmp(role, "video_decoder.avc") == 0) {
        ...
        bNeedCopyCodecDataToDataBuffer = OMX_TRUE;
        fsl_osal_strcpy((fsl_osal_char*)name, LIBAV_COMP_NAME_AVCDEC);
...
FilterBufRetCode LibavVideoDec::DecodeOneFrame()
{
    ...
    // workaround for core parser output AVC format.
    if(bNeedCopyCodecDataToDataBuffer) {

        fsl_osal_memmove((fsl_osal_ptr)((OMX_U32)pInBuffer + nCodecDataLen), \
                pInBuffer, nInputSize);
        fsl_osal_memcpy(pInBuffer, pCodecData, nCodecDataLen);
