...
OMX_ERRORTYPE AndroidAudioRender::DeviceDelay(OMX_U32 *nDelayLen)
{
    ...
    if (res == OK) {
        ...
        //printf("nWrited %d, numFramesPlayed: %d, nowUs %lld, numFramesPlayedAt %lld", nWrited, numFramesPlayed, nowUs, numFramesPlayedAt);
        //printf("sampleSize %d, sampleingRate %d, nBufferSize %d, nPSize %d", nSampleSize, PcmMode.nSamplingRate, nBufferSize, nPSize);
...
OMX_ERRORTYPE AndroidAudioRender::ResetDevice()
{
    ...
    if (bOpened == OMX_TRUE) {
        ...
        LOG_DEBUG("ResetDevice: SampleRate: %d, Channels: %d, nClockScale %x\n",
                PcmMode.nSamplingRate, nChannelsOut, nClockScale);
